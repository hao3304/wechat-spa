...
~function ($) {
    ...
    validateReturn = function ($item, klass, parent, message) {
        ...
        if($item.val()==""&&$item.attr("validator")){
    ...
    validate = function ($item, klass, parent) {
        ...
        if (!(/^(?:radio|checkbox)$/.test(type) || aorb) && !patterns.text(val)) {
            ...
            var bool = val.length ? 'unvalid' : 'empty';
            if($item.attr("validator")){

                bool = "unvalid";
            }

            return validateReturn.call(this, $item, klass, parent, bool)
        }
    ...
    $.fn.validator = function (_options) {
        var $form = this
            , options = _options || {}
            , identifie = options.identifie || '[required],[validator]'
